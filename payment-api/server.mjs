...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.method==='POST'&&req.url==='/v1/payments'){const input=JSON.parse(await body(req)||'{}');const plan=getPlan(input.plan);const email=String(input.email||'').trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))throw Object.assign(new Error('VALID_EMAIL_REQUIRED_FOR_RECEIPT'),{status:400});const id=`SD-${new Date().toISOString().slice(0,10).replace(/-/g,'')}-${randomUUID().slice(0,8).toUpperCase()}`;await putOrder({id,plan:plan.id,amount:plan.amount,status:'pending',taskId:String(input.taskId||'').slice(0,80),receiptEmail:email,createdAt:new Date().toISOString()});const payment=await createTochkaPayment({...env,PUBLIC_ORIGIN:origin},{orderId:id,plan,email});await patchOrder(id,{provider:'tochka',paymentUrl:payment.url,operationId:payment.operationId||'',consumerId:payment.consumerId||'',fiscalization:'tochka_receipt'});return json(res,201,{ok:true,orderId:id,paymentUrl:payment.url})}
