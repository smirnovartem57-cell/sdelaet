...
export default {
  async fetch(request,env){
    ...
    if(url.pathname==='/health')return json({ok:true,service:'sdelaet-search-api',providers:{yandexSearch:Boolean(env.YANDEX_SEARCH_API_KEY&&env.YANDEX_FOLDER_ID),dgis:Boolean(env.DGIS_API_KEY)}},200,origin,env);
    if(url.pathname!=='/v1/candidates/search'||request.method!=='POST')return json({ok:false,error:'NOT_FOUND'},404,origin,env);
    let body={};try{body=await request.json()}catch{return json({ok:false,error:'BAD_JSON',message:'Некорректный JSON.'},400,origin,env)}
    if(textSize(body)>12000)return json({ok:false,error:'PAYLOAD_TOO_LARGE'},413,origin,env);
    try{
      const result=await searchCandidates(env,body);
      return json(result,result.status||200,origin,env);
    }catch(e){
      return json({ok:false,error:'SEARCH_FAILED',message:'Не удалось выполнить поиск. Попробуйте ещё раз.',detail:env.DEBUG==='1'?String(e?.message||e):undefined},502,origin,env);
    }
  }
};
