...
- `INTERIOR_DOORS` — `TESTING`, research PASS, Agent Pipeline PASS, QA 15/15 PASS, E2E PASS, profile v1.0.
- `STRETCH_CEILING` — `TESTING`, research PASS, Agent Pipeline PASS, QA 15/15 PASS, E2E PASS, profile v1.0.
...
Dev-time pipeline из 9 независимых ролей введён в проект: research, domain expert, intake, contractor brief, comparison, consistency, technical QA, regression и release controller. Новые категории должны проходить `node tools/category-agents/run.mjs --all --ci`; внешняя LLM не является обязательной зависимостью.
