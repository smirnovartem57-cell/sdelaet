...
const reviewedEvidence={...evidence,cannibalizationReviews:research.cannibalization.map(x=>({query:x.query,otherCategoryId:x.otherCategoryId,disposition:'false_positive',ownerCategoryId:'balcony-insulation',reason:'Regression review'}))};
const reviewed=buildResearch(category,manifest.categories,reviewedEvidence);
check('reviewed conflicts are resolved',reviewed.unresolvedCannibalization.length===0);
check('reviewed research becomes ready for review',reviewed.status==='READY_FOR_REVIEW');
