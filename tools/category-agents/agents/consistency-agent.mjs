...
export function run(ctx){
  ...
  if(strict){const ar=audit(),deploy=read('.github/workflows/deploy-site-vds.yml'),ready=read('tools/platform-readiness.mjs'),manifestGate=deploy.includes('node tests/category-regression-manifest.mjs')||(deploy.includes('node tools/platform-readiness.mjs')&&ready.includes('tests/category-regression-manifest.mjs'));checks.push(['QA test',!!m?.qaTest&&exists(m.qaTest)],['E2E test',!!m?.e2eTest&&exists(m.e2eTest)],['generated runtime registry',read('assets/category-manifest.generated.js').includes(p.serviceCode)],['generated search registry',read('search-api/category-config.generated.mjs').includes(`"${p.categoryId}"`)],['actual system audit coverage',ar.ok&&ar.stdout.includes(`PASS ${p.serviceCode} profile identity`)],['manifest regression runner in CI',manifestGate]);}
